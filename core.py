...
def clean_tweets(a, pos_words, pos_emoji, neg_emoji, neg_words, obscene):
    ...
    result = re.sub(
        r'[a-zA-Z.,?!@#$%^&*()_+-]+', ' ', result
    )  # англ слова и символы
    result = result.lower()  # приведение к низкому регистру
    result = re.sub(r'\s+', ' ', result)  # лишние пробелы
    clean_tweet = ' '.join(
        word for word in result.split() if len(word) > 2
    )  # удаление слов длинной 1,2 символа
...
def term_freq(x, vector):
    ...
    word_freq_df['frequency'] = \
        word_freq_df['occurrences'] / np.sum(word_freq_df['occurrences'])
    word_freq_df.sort_values(
        'occurrences',
        ascending=False
    ).to_csv('data/term_frequences.csv')
